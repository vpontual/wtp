...
function loadSettings() {
  try {
    ...
    const defaults = { congress: 118, session: 2, includeHouse: true, includeSenate: true };
    return raw ? { ...defaults, ...JSON.parse(raw) } : defaults;
  } catch {
    return { congress: 118, session: 2, includeHouse: true, includeSenate: true };
  }
}
...
async function fetchJson(url, label = "") {
  ...
  if (!res.ok) {
    throw new Error(`${label || 'URL'} failed (HTTP ${res.status}). ${text.slice(0, 180)}`.trim());
  }
  
  try {
      return JSON.parse(text);
  } catch (e) {
      throw new Error(`${label || 'URL'} returned invalid JSON. Response text: ${text.slice(0, 180)}`);
  }
}
...
async function fetchXml(url, label = "") {
  ...
  if (err) throw new Error(`${label || 'URL'} XML parse error: ${err.textContent}`);
  return doc;
}
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-md mb-8">
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 items-end">
        {/* Congress Input */}
        <div>
          <label htmlFor="congress" className="block text-sm font-medium text-gray-300">Congress</label>
          <input
            type="number"
            name="congress"
            id="congress"
        ...
        {/* Session Input */}
        <div>
          <label htmlFor="session" className="block text-sm font-medium text-gray-300">Session</label>
          <input
            type="number"
            name="session"
            id="session"
        ...
        <div className="flex items-center space-x-4 pt-6">
          <div className="flex items-center">
            <input id="includeSenate" name="includeSenate" type="checkbox" checked={settings.includeSenate} onChange={handleInputChange} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-500 rounded bg-gray-700" />
            <label htmlFor="includeSenate" className="ml-2 block text-sm text-gray-300">Senate</label>
          </div>
          <div className="flex items-center">
            <input id="includeHouse" name="includeHouse" type="checkbox" checked={settings.includeHouse} onChange={handleInputChange} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-500 rounded bg-gray-700" />
            <label htmlFor="includeHouse" className="ml-2 block text-sm text-gray-300">House</label>
          </div>
        </div>
        {/* Action Buttons */}
        <div className="md:col-span-3 lg:col-span-2 flex items-center justify-end space-x-3">
          ...
          >
            {loading ? (
              <>
                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 M 24 24">
...
function App() {
  ...
  const fetchVotes = useCallback(async () => {
    if (!settings.includeHouse && !settings.includeSenate) {
      setError("Please select at least one chamber (House or Senate) to fetch votes.");
      setVotes([]);
      return;
    }
    ...
    try {
      ...
      if (settings.includeSenate) {
        ...
        if (data.roll_calls && data.roll_calls.roll_call) {
            ...
            console.warn("Senate data format unexpected or no roll calls found.", data);
        }
      }
      ...
      if (settings.includeHouse) {
        ...
        const houseVotes = Array.from(doc.querySelectorAll("rollcall-vote")).map(v => {
            const date = v.querySelector("action-date")?.textContent + "T" + v.querySelector("action-time")?.textContent;
            return {
              chamber: 'House',
              number: v.querySelector("rollcall-num")?.textContent,
              date: date,
              title: v.querySelector("vote-question")?.textContent || "N/A",
              result: `${v.querySelector("vote-result")?.textContent} (${v.querySelector("totals-by-vote > yea-total")?.textContent}-${v.querySelector("totals-by-vote > nay-total")?.textContent})`,
    ...
    <div className="bg-gray-900 text-white min-h-screen font-sans">
      <div className="container mx-auto p-4 md:p-8">
        <header className="mb-8">
          <h1 className="text-4xl font-bold text-center text-indigo-400">Congress Vote Tracker</h1>
          <p className="text-center text-gray-400 mt-2">
            View the latest roll call votes from the U.S. Senate and House of Representatives.
          </p>
        </header>

        <SettingsPanel settings={settings} setSettings={setSettings} onFetch={fetchVotes} loading={loading} />
        ...
        {!loading && votes.length === 0 && !error && (
            <div className="text-center py-16 px-6 bg-gray-800 rounded-lg">
                <h3 className="text-xl font-semibold text-gray-300">No Votes Loaded</h3>
        ...
        <div className="space-y-4">
          {votes.map((vote) => (
            <div key={vote.key} className="bg-gray-800 shadow-lg rounded-lg p-5 transition-transform transform hover:scale-[1.02]">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-3">
                <div className="flex items-center space-x-3 mb-2 sm:mb-0">
                    <span className={classNames(
                        "font-bold text-sm px-3 py-1 rounded-full",
                        vote.chamber === 'Senate' ? 'bg-blue-800 text-blue-200' : 'bg-green-800 text-green-200'
                    )}>
                        {vote.chamber}
                    </span>
                    <p className="text-gray-400">Vote #{vote.number}</p>
                </div>
                <p className="text-sm text-gray-500 flex-shrink-0">{fmtDate(vote.date)}</p>
              </div>
              <h2 className="text-lg font-semibold text-gray-200 mb-2">{vote.title}</h2>
              <p className="text-md font-mono bg-gray-900 px-3 py-1 rounded-md inline-block text-indigo-300">{vote.result}</p>
            </div>
          ))}
        </div>
      </div>
